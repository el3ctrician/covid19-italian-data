...
import requests
PCM_DPC_URL = "https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-json/dpc-covid19-ita-andamento-nazionale.json"
...
response = requests.get(PCM_DPC_URL)


for obj in response.json():
  date=dt.datetime.strptime(obj['data'],'%Y-%m-%dT%H:%M:%S').strftime("%m/%d")
  dates.append(date)
...
axs[0,0].plot(dates,tested,'-oc',ms=4)
...
for plot in axs.flatten():
  plot.tick_params(axis='x', which='major', labelsize=8, rotation=45)

plt.subplots_adjust(wspace=0.1, hspace=0.4)
multi = MultiCursor(fig.canvas,axs.flatten(),horizOn=False, color='r', lw=1)
